...
- `analyze`: 行・列・原文の文字オフセットと理由をJSONで返す。オフセットはUnicodeコードポイント単位。`metrics.style`に文書全体の傾向（`report`と同じ数値）を含める。
- `fix`: 文頭の「まず最初に」と、形態素条件を満たす「サ変名詞＋することができます」のみ短縮候補を保存する。他の表現はエージェントが判断する。原文と既存出力を上書きしない。
- `verify`: 数字と一部の単位、ASCII用語、辞書が固有名詞とした語、指定語、保護領域の出現回数を照合する。否定・可能・義務などの表現差と、根拠のない前提になりやすい言い方（`unsourced`：一般化・伝聞・読み手の気持ちの推測・ぼかした体験談）の増減を確認対象として返す。
- `verify --condense`: 要約・圧縮向けの照合。語や数値の出現回数ではなく有無で比べ、原文にない語や数値が新しく現れた変化だけを`changes`（終了コード1）にする。消えた語は`dropped`に入れ、止めない。
- `report`: 修正前後を比べた評価シートをMarkdownで返す。概要（文字数・指摘件数・照合結果）、文書全体の傾向（文の数、20文字未満の文・一文だけの行・箇条書きの行の割合。コードブロック・見出し・表・空行は数えない）、変更した文ごとの前後と対応した指摘、解消・残存・新規の指摘、機械照合の差異を含む。変更の対応づけは文単位で、文の分割・統合は一つの行にまとめて示す。
