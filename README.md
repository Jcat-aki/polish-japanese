...
| エージェントによる一回の言い換えと、主張についての相談（最大一つ）・書き手しか知らない事実の確認（最大三つ） | AI 検出器の回避の保証 |
...
| `vague-degree` | 「非常に」「大幅に」「かなり」など、どのくらいかを数字や例で示していない程度の言葉（同じ文に数字があれば対象外）。中身は書き手しか知らないので、書き手に尋ねる対象として返す |
| `binary-contrast` | 「単なるAではなく、B」「AであってBではない」「Aではなく、B」のように、対比で主張を立てる型（「雨ではなく晴れ」のような名詞の選択は対象外） |
| `negative-listing` | 「Aでもない、Bでもない」と否定を重ねる型 |
| `false-agency` | 「データが示している」「歴史が物語っている」「浮き彫りになる」「醸成される」のように、モノや抽象が人の動作をする言い方 |
| `era-opener` | 「AIの進化が目覚ましいこの時代において」「変化の激しい時代に」「AI時代において」のように、時代を持ち出す決まった書き出し（「江戸時代」「学生時代」のような具体的な時代は対象外） |
| `symbol-artifact` | 全角ダッシュ（——・──）、閉じていない `**`、装飾の絵文字（🚀✨💡など） |
| `katakana-metaphor` | 「思考のOS」「習慣をインストール」「人生をハック」のような横文字の比喩 |
| `pet-word` | 「解像度を上げる」「手触り」「熱量」「営み」「腹落ち」のような、AIが好んで使う語 |
| `academic-self` | 「本稿」「本記事」「筆者」のような論文風の自称 |
| `self-declared-importance` | 「ここで重要なのは」「大切なことは」「本質は」「ポイントは」「これが最も重要です」「ここからが本題」「ここに〜の核心があります」「そこで重要だったのが」「鍵を握るのは」のように、重要さを自分で宣言する言い方 |
...
各指摘は「文脈の漏れ・根拠のない前提」「AIに多い型」「強調・誇張」「読みやすさ」の4つに分類され、評価シートには分類ごとの件数が修正前後で並びます。「圧倒的」「本質は」のような強調・誇張は、応募文や宣伝文では書き手が意図して選ぶことがあるので、AIに多い型とは分けて扱います。
