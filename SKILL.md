...
9. 言い換え案を先に返して終了する。自動の再診断・再推敲ループは回さない。希望があれば修正理由を短く示す。主張の相談は最大一つに絞り、必要なときだけ添える。残った文体上の粗さを理由に回答を遅らせない。
...
全コマンドはJSONを標準出力へ返す。Pythonコードは外部APIへ本文を送らない。推敲本文の生成はこのスキルを使うエージェントが担う。`fix`は全自動の文章生成器ではない。
