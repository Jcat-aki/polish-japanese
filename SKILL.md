...
`findings`は確認すべき場所であり、AIが書いた証拠ではない。辞書は固有名詞候補の保護と名詞の連なりの検出を助けるが、意味理解や辞書の完全性を保証しない。ユーザー指定語は`--keep`で補う。常に保護する語は`--keep-file`（1行1語、`#`はコメント）か環境変数`POLISH_KEEP_FILE`で渡せる。ラッパーは`~/.config/polish-japanese/keep.txt`があれば自動で渡す。
